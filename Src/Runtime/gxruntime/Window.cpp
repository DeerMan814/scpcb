...
Window::Window(String c, int w, int h, bool fs) {
    caption = c;
...
void Window::swap(bool vsyncEnabled) {
    dxSwapChain->Present(vsyncEnabled ? 1 : 0, 0);
}

ID3D11Device* Window::getDxDevice() const {
    return dxDevice;
}

ID3D11DeviceContext* Window::getDxContext() const {
    return dxContext;
}

ID3D11RenderTargetView* Window::getBackBufferRtv() const {
